...
selected_col = ['A-MMD', 'B-MMD', 'DTABR(global)', 'DTABR(frontal)', 'D-MFO']
selected_col1 = ['A-MMD', 'D-MFO', 'B-MMD', 'DTABR(global)', 'DTABR(frontal)']
origin_data = [59.84, 59.12, 1.59, 2.11, 3.31]
...
def prefun():
    ...
    with st.expander("**Predict result**", True):
        st.info("Predict output_margin value is: {res},  $Probability=\\frac {1} {1 + e^{(-output\_margin)}}$, output_margin segmentation threshold is **0.0331**, probability segmentation threshold is **50.83%**.".replace("{res}", str(round(float(res), 4))))
        st.markdown(f'''
